...
const AllTestimonials = () => {
  ...
  const [cat, setCat] = useState({ client: "", image: "", feedback: "" });
  const { client, image, feedback } = cat;
  const [error, setError] = useState("");
  const [show, setShow] = useState(false);
  const [cate, setCate] = useState({ client: "", image: "", feedback: "" });
  ...
  const getAllData = async () => {
    const responce = await axios.get("http://localhost:3003/testimonials");
  ...
  const handleSubmit = async (e) => {
    ...
    if (!client || !feedback) {
      ...
      await axios.post("http://localhost:3003/testimonials", cat);
      console.log(cat);
      navigate("/allcat");
      setError("");
  ...
  const getData = async (id) => {
    const responce = await axios.get(
      `http://localhost:3003/testimonials/${id}`
    );
  ...
  const handleUpdate = async (e) => {
    ...
    if (!client || !feedback) {
      ...
      await axios.put(`http://localhost:3003/testimonials`, data);
      console.log(data);
      setError("");
      window.location.reload(true);
    }
  ...
  const handleDelete = (id) => {
    if (window.confirm("Are you sure want to delete?")) {
      axios.delete(`http://localhost:3003/testimonials/${id}`);
